...
namespace FinanceSystem.Web.Controllers
{
    ...
    public class FinancingsController : Controller
    {
        ...
        public async Task<IActionResult> ApplyCorrection(string id)
        {
            ...
            {
                var token = HttpContext.GetJwtToken();
                var financing = await _financingService.GetFinancingByIdAsync(id, token);
            ...
            {
                TempData["ErrorMessage"] = "Erro ao preparar aplicação de correção: " + ex.Message;
                return RedirectToAction(nameof(Details), new { id });
            }
        }
        ...
        public async Task<IActionResult> ApplyCorrection(ApplyCorrectionModel model)
        {
            if (!ModelState.IsValid)
            {
                ...
                {
                    var token = HttpContext.GetJwtToken();
                    var financing = await _financingService.GetFinancingByIdAsync(model.FinancingId, token);
                    ViewBag.Financing = financing;
                }
                catch
                {
                    // Ignorar erro, voltaremos para a página de detalhes
                }
            ...
            {
                var token = HttpContext.GetJwtToken();
                await _financingService.ApplyMonetaryCorrectionAsync(model, token);
                TempData["SuccessMessage"] = "Correção monetária aplicada com sucesso";
                return RedirectToAction(nameof(Details), new { id = model.FinancingId });
            }
            catch (Exception ex)
            {
                TempData["ErrorMessage"] = "Erro ao aplicar correção monetária: " + ex.Message;
                return RedirectToAction(nameof(Details), new { id = model.FinancingId });
            }
        }

        [RequirePermission("financings.forecast")]
        public async Task<IActionResult> Forecast(string id)
        {
            ...
            try
            {
                ...
                ViewBag.Financing = financing;
                ViewBag.DefaultMonths = 12; // Padrão para 12 meses

                return View();
            }
            catch (Exception ex)
            {
                TempData["ErrorMessage"] = "Erro ao preparar previsão: " + ex.Message;
                return RedirectToAction(nameof(Details), new { id });
            }
        }
        ...
        [ValidateAntiForgeryToken]
        [RequirePermission("financings.forecast")]
        public async Task<IActionResult> Forecast(string id, int forecastMonths)
        {
            if (string.IsNullOrEmpty(id))
            {
                return BadRequest("ID do financiamento não fornecido");
            ...
            try
            {
                ...
                var forecast = await _financingService.ForecastFinancingAsync(id, forecastMonths, token);
